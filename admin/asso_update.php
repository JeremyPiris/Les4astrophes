...
$imgMembre = $prenomMembre = $roleMembre = $groupeMembre = $imgErrorMembre = $prenomErrorMembre = $roleErrorMembre = $groupeErrorMembre = "";
  ...
  if(!empty($_POST))
  {
    ...
    if(($isSuccess && $isImageUpdated && $isUploadSuccess) || ($isSuccess && !$isImageUpdated))
    {
      ...
      if($isImageUpdated)
      {
        $statement = $db->prepare("UPDATE membres_asso set photo_membre = ?, prenom_membre = ?, poste_membre = ?, groupe_membre = ?
                                    WHERE id = ?");
        $statement->execute(array($imgMembre, $prenomMembre, $roleMembre, $groupeMembre, $id));
      }
      else {
        $statement = $db->prepare("UPDATE  membres_asso set prenom_membre = ?, poste_membre = ?, groupe_membre = ?
                                    WHERE id = ?");
        $statement->execute(array($prenomMembre, $roleMembre, $groupeMembre, $id));
      }
      Database::disconnect();
    ...
    $prenomMembre = $item['prenom_membre'];
    $roleMembre = $item['poste_membre'];
    $groupeMembre = $item['groupe_membre'];
    Database::disconnect();
  }
...
<html lang="fr" dir="ltr">
  ...
  <body>
    ...
    <div class="container admin">
      ...
      <form class="form" role="form" action="<?php echo 'asso_update.php?id=' . $id; ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="row">
          ...
          <div class="col-sm-6">
              ...
              <div class="form-group">
                <label for="groupeMembre">Poste :</label>
                <input type="text" class="form-control" id="groupeMembre" name="groupeMembre" placeholder="Groupe" value="<?php echo $groupeMembre; ?>">
